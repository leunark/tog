...
function onGlobalFilter(table, event) {
...
<template>
    ...
    <div class="layout-banking-dashboard">
        <div class="grid">
            <Message class="w-full border-round-3xl surface-card font-medium text-color-secondary -mt-1" :class="{ 'dark-mode': isDarkMode }"> 👋 Hello! Welcome to Freya! Before start please complete your profile to know you better.</Message>
            <div class="col-12">
                    ...
                    <div class="flex flex-wrap gap-2 sm:ml-auto justify-content-center">
                        <div class="exchnage-rates flex">
                            ...
                            <OverlayPanel ref="op" :style="{ width: '300px' }">
                                ...
                                <Chart type="bar" :height="180" :data="usdChartData" :options="chartOptions"></Chart>
                            </OverlayPanel>
                            ...
                            <OverlayPanel ref="op2" :style="{ width: '300px' }">
                                ...
                                <Chart type="bar" :height="180" :data="btcChartData" :options="chartOptions"></Chart>
                            </OverlayPanel>
                            ...
                            <OverlayPanel ref="op3" :style="{ width: '300px' }">
                                <template>
                                    ...
                                    <Chart type="bar" :height="180" :data="poundChartData" :options="chartOptions"></Chart>
                                </template>
                            </OverlayPanel>
                        </div>
                        <div class="flex gap-2">
                            <Button type="button" v-tooltip="'Exchange'" placeholder="bottom" icon="pi pi-arrows-h" class="p-button-rounded p-button-outlined flex-shrink-0"></Button>
                            <Button type="button" v-tooltip="'Withdraw'" placeholder="bottom" icon="pi pi-download" class="p-button-rounded p-button-outlined flex-shrink-0"></Button>
                            <Button type="button" v-tooltip="'Send'" placeholder="bottom" icon="pi pi-send" class="p-button-rounded flex-shrink-0"></Button>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="grid" style="margin: -1rem">
                    ...
                    <div class="col-12 p-3">
                        <div class="card">
                            ...
                            <Chart type="bar" :height="165" :data="chartData" :options="chartOptions"></Chart>
                        </div>
                    </div>
                </div>
                ...
                >
                    ...
                    <div class="flex justify-content-between align-items-center gap-7">
                        ...
                        <div class="px-3 xl:px-5 py-5 w-full border-round-2xl shadow-3" style="background: linear-gradient(to left bottom, var(--primary-100), var(--primary-400)); min-height: 19rem">
                            <div class="mb-5">
                                <img src="/layout/images/logo-freya-single.svg" class="w-2rem" alt="" />
                            </div>
                            <div class="mb-3">
                    ...
                    <template #footer>
                        <Button icon="pi pi-check" @click="addCreditCard()" label="Save Card" class="p-button-text"></Button>
                    </template>
                </Dialog>
                <div class="grid">
                    <div class="col-12 md:col-6 xl:col-12 p-3">
                        <div class="card px-0" v-if="cards">
                            <div class="card-header gap-3" style="padding: 0 28px 16px">
                                ...
                                <Button v-ripple type="button" icon="pi pi-plus" class="p-button-secondary p-button-text p-button-rounded" @click="showBasicDialog()"></Button>
                            </div>
                        ...
                        <div class="card" v-if="cards">
                            <div class="card-header gap-3">
                                ...
                                <Dropdown :options="cards" v-model="selectedCard" :placeholder="selectedCard.cardNo" optionLabel="cardNo" :showClear="false" class="w-8rem" panelclass="w-8rem">
                                    <template let-card="item">
                                        <span class="block">****{{ cards.cardNo.split('-')[3] }}</span>
                                    </template>
                                </Dropdown>
                            </div>
                            <ConfirmDialog
                            ...
                            <TabView>
                                <TabPanel header="Send Money">
                                    <div style="margin: 0 -1.5rem" class="w-full p-0">
                                        ...
                                        <Button v-ripple icon="pi pi-send" type="button" label="Send" class="p-button-outlined w-full" @click="confirm1(selectedAccount.name ? selectedAccount.name : selectedAccount, amount)"></Button>
                                    </div>
                                </TabPanel>
                    ...
                    <div class="card-header gap-3 flex-wrap md:flex-nowrap">
                        ...
                        <div class="inline-flex align-items-center w-full md:w-auto">
                            <span class="p-input-icon-left flex-auto">
                                ...
                                <InputText type="text" @input="onGlobalFilter(dt, $event)" placeholder="Search" class="w-full" style="border-radius: 2rem" />
                            </span>
                            <Button v-ripple icon="pi pi-upload" class="p-button-rounded mx-3 flex-shrink-0" @click="dt.exportCSV()"></Button>
                        </div>
                    </div>
                    <DataTable :value="transactions" :paginator="true" :rows="3">
                        <Column>
                        ...
                        <Column>
                            <template #body="slotProps">
                                <Button icon="pi pi-ellipsis-v" class="p-button-rounded p-button-text" @click="slotProps.menu.toggle($event)"></Button>
                            </template>
                        </Column>
                    </DataTable>
                </div>
            ...
            <div class="col-12 sm:col-6 xl:col-3 p-3">
                <div class="card h-full flex align-items-center justify-content-start p-5" style="background: linear-gradient(to left bottom, var(--primary-lighter-color), var(--primary-400))">
                    <div class="flex flex-column">
                        ...
                        <Button v-ripple icon="pi pi-wallet" type="button" label="Upgrade Now" class="p-button-primary w-full p-button-lg"></Button>
                    </div>
                </div>
            </div>
        </div>
...
:deep(.p-message .p-icon:not(.p-message-close-icon)) {
    display: none !important;
}
::v-deep(.dark-mode .p-message-close-icon) {
