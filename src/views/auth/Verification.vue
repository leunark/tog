...
const onDigitInput = (event: KeyboardEvent) => {
            ...
            <div class="flex flex-column align-items-center justify-content-center w-full md:w-4 h-full text-center py-6 px-4">
                ...
                <div class="flex flex-column">
                    <div class="flex justify-content-between w-full align-items-center mb-4 gap-3">
                        <InputText v-model="val1" p-key-filter="num" class="w-3rem text-center text-color-secondary surface-50 border-200" maxlength="1" @keyup="onDigitInput($event)" />
                        <InputText v-model="val2" p-key-filter="num" class="w-3rem text-center text-color-secondary surface-50 border-200" maxlength="1" @keyup="onDigitInput($event)" />
                        <InputText v-model="val3" p-key-filter="num" class="w-3rem text-center text-color-secondary surface-50 border-200" maxlength="1" @keyup="onDigitInput($event)" />
                        <InputText v-model="val4" p-key-filter="num" class="w-3rem text-center text-color-secondary surface-50 border-200" maxlength="1" @keyup="onDigitInput($event)" />
                    </div>

                    <div class="flex flex-wrap gap-2 justify-content-between">
                        <Button v-ripple label="Cancel" class="flex-auto p-button-outlined" @click="navigateToDashboard"></Button>
                        <Button v-ripple label="Verify" class="flex-auto" @click="navigateToDashboard"></Button>
                    </div>
                </div>
            </div>
